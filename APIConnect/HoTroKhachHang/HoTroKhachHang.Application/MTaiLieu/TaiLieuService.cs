...
namespace HoTroKhachHang.Application.MTaiLieu
{
    public class TaiLieuService : ITaiLieuService
    {
        ...
        public async Task<List<ProductDTO>> SanPhamTheoKhachHang()
        {
            //var session = new SessionAPIAsync();
            var temp = await IdentityGlobalServiceToken.session.GetProductByCustomer(IdentityGlobalServiceToken.Token, IdentityGlobalServiceToken.UserID);
            return temp;
        }

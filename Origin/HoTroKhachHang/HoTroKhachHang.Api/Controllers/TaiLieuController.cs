...
namespace HoTroKhachHang.Api.Controllers
{
    ...
    public class TaiLieuController : ControllerBase
    {
        ...
        //start-test
        public ObjectResult testSanPhamTheoKhachHang(List<Test_SanPhamTheoKhachHang> data)
        {
            dynamic foo = new ExpandoObject();
            ResponseOutput output = new ResponseOutput();
            try
        ...
        [Authorize(Roles = "Khách hàng")]
        public async Task<IActionResult> testSanPhamTheoKhachHang(string makhachhang)
        {
            var result = await _TaiLieuService.testSanPhamTheoKhachHang(makhachhang);
            return testSanPhamTheoKhachHang(result);
        }
